...
if '404_URL_list.txt' not in file_list:
    with open('404_URL_list.txt', 'w') as f:
        f.write('\n')

if 'checked_URL_list.txt' not in file_list:
    with open('checked_URL_list.txt', 'w') as f:
        f.write('\n')
    ...
    while i < len(urls['code']) :
        ...
        with Browser('firefox', profile_preferences=prof, headless=True) as new_browser:
            ...
            if new_browser.is_element_not_present_by_css('.fyre-livecount'):
                ...
                if(new_browser.is_element_present_by_text('Conversation (')):
                    listening = new_browser.find_by_text('Conversation (')
